# .archive/

Carpeta para material histórico del repo (checkpoints viejos, _archive, reference_old, backups).

**No usar para trabajo actual.**

Todo lo vivo está en:
- context/flujo_hub.html (main)
- context/svg_visualizer.html
- context/plano_demo.html
- src/
- projects/ (alineados a aistetic)
- docs/ (solo los recomendados: AGENT_OPERATING_MANUAL, REPO_MAP, HIGIENE...)

Política: mover aquí lo viejo para mantener la raíz limpia (ver docs/HIGIENE_REPO.md).

Este README fue creado para guiar la higiene del repo general.
