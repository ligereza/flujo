    ...
    if download_post:
        try:
            ...
            if result["status"] == "downloaded":
                ...
                data["instagram"]["media_type"] = result.get("media_type", "")
                data["instagram"]["file_count"] = result.get("file_count", 0)
                data["instagram"]["owner"] = result.get("owner", "")
                data["instagram"]["date_utc"] = result.get("date", "")
                ...
                # Usar owner como producer sugerido si está vacío
                if result.get("owner") and not data["extracted_info"].get("producer"):
...
def main():
    ...
    for i, item in enumerate(items, start=1):
        ...
        # Nombre de proyecto más descriptivo: ig_<shortcode>
        name = f"ig_{code}"
